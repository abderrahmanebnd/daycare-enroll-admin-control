  ...
  useEffect(() => {
    if (searchTerm.trim() === '') {
      ...
      const filtered = media.filter(item => 
        ...
        getUserName(item.uploadedBy).toLowerCase().includes(lowerSearchTerm) ||
  ...
  const fetchData = async () => {
    try {
      ...
      if (user.role === 'parent') {
        ...
        const childrenWithConsent = children.filter(child => child.mediaConsent);
        ...
        {filteredMedia.length === 0 ? (
          <div className="text-center p-12 bg-muted/20 rounded-lg">
            <p className="text-muted-foreground">
              {searchTerm ? 'Aucun média ne correspond à votre recherche' : 
               user?.role === 'parent' ? 'Aucun média disponible pour vos enfants' : 
              ...
              <div key={item.id} className="overflow-hidden rounded-lg border bg-card shadow-sm">
                ...
                <div className="p-4">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-medium">{getChildName(item.childId)}</h3>
                    <span className="text-xs text-muted-foreground">
                      {new Date(item.createdAt).toLocaleDateString()}
                    </span>
                  </div>
                  <p className="text-sm text-muted-foreground mb-2">{item.description}</p>
                  <p className="text-xs text-muted-foreground">Par: {getUserName(item.uploadedBy)}</p>
                </div>
              </div>
            ))}
          </div>
